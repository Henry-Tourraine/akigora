...
class Operations:
    ...
    def filtre(df, colonnes):
        comparateur = {"inf": lambda name, types: lambda x: x[name] < types,
                       "sup": lambda name, types: lambda x: x[name] > types}
    ...
    def somme(df, colonne, result=None, value=None):
        df = Operations.filtre(df, colonne)
        print(f"somme df before{df.columns} type : {df[df.columns[0]].dtype }")
        return sum(df)
    ...
    def process(engineer_descriptions, df, filters=[]):  # filters for interactivity
        ...
        for description in engineer_descriptions:
            ...
            if len(colonnes) > 0 and colonnes[0]["name"] == "result":
                ...
                result = Operations.operations[operation['fonction']](df, col_list, result=result,
